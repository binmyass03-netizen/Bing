-- DELTA FIX: stub missing executor functions
...
if not raknet then raknet = {add_send_hook=function()end,remove_send_hook=function()end,desync=function()end} end
if not buffer then buffer = {writeu32=function()end,writei32=function()end,writeu8=function()end} end
...
pcall(function()
    if raknet and raknet.add_send_hook then
        raknet.add_send_hook(function(packet)
            if packet.PacketId == 0x1B then
                local ok2, data = pcall(function() return packet.AsBuffer end)
                if ok2 and data and buffer and buffer.writeu32 then
                    buffer.writeu32(data, 1, 0xFFFFFFFF)
                    pcall(function() packet:SetData(data) end)
                end
            end
        end)
    end
